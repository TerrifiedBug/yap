...
final class Daemon: NSObject, NSApplicationDelegate {
    ...
    private func wire(_ detector: MeetingDetector) {
        detector.onMeetingStart = { [weak self, weak detector] pid, app in
            guard let self else { return }
            ...
            if let app, Config.meetingExcludedApps().contains(app.bundleID) {
                warn("◇ \(app.name) is excluded — ignoring")
                // Drops the client out of the detector's scan, so it neither
                // re-fires every poll nor hides another app behind it.
            ...
            let ignoreApp: @MainActor () -> Void = { [weak self, weak detector] in
                ...
                Config.update { config in
                    ...
                    if !list.contains(app.bundleID) { list.append(app.bundleID) }
                    config["meeting_excluded_apps"] = list
                }
                warn("◇ \(app.name) added to the ignore list")
                detector?.ignoreCurrentClient()
            ...
            if Config.meetingAutoRecord() {
                ...
                showToast(
                    title: app.map { "Recording \($0.name) call" } ?? "Recording meeting",
                    body: title ?? "Stop from the menu bar or here",
                    button: "Stop",
                    secondaryButton: app.map { _ in "Ignore" },
            ...
            askUser(
                title: app.map { "\($0.name) is in a call" } ?? "Your microphone is in use",
                body: title ?? "Record this meeting?",
                button: "Record",
                // Only when macOS names the client at all: a bare executable
                // with no bundle behind it has no identity we could store, so
                // it gets Dismiss and no more.
                secondaryButton: app.map { "Ignore \($0.name)" },
                onSecondary: ignoreApp,
